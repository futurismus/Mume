...
/**
 * MumePlex: A storage and retrieval system for Mume spectral data.
 */
public class MumePlex extends MaxObject {
    ...
    public void recall(Atom[] args) { lookup(args); }
    public void size() { updateCount(); }
    ...
    public void anything(String msg, Atom[] args) {
        if (msg.equals("recall")) recall(args);
        else if (msg.equals("getvector")) {
            if (args.length > 0) {
                if (args[0].isString()) lookup(new Atom[]{args[0], Atom.newAtom("vector")});
                else if (args[0].isInt()) get(new Atom[]{args[0], Atom.newAtom("vector")});
            }
        } else if (msg.equals("dump")) dump();
        else if (msg.equals("clear")) clear();
        else if (msg.equals("getkeys")) getkeys();
        else if (msg.equals("size")) size();
    }
}
